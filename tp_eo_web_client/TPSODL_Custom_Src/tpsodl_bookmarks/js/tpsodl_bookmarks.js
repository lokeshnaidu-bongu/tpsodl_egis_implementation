...
define([
    ...
    function Bookmark($, _, Messages, template, ol, hogan, CommsLayer) {
            ...
            $('#bookmarks_table').DataTable({
                ...
                buttons: [

                    {
                        ...
                        text: '<b style="padding:none !important;font-size:10px;margin-left: 2px !important;" title="Delete Bookmark">Delete</b>',
						// text : '<button class="btn"><b>Delete</b></button>',
                    ...
                    },
                    {
                        ...
                        text: '<b style="padding:none !important;font-size:10px;margin-left: 2px !important;" title="Goto Bookmark">GoTo</b>',
                        action: function(e, dt, node, config) {
                            gotoBookmark();
                        }
                    },
                    {
                        ...
                        // text: '<i class="icon-bookmark" title="Create Bookmark"></i>',
                        text: '<b style="padding:none !important;font-size:10px;margin-left: 2px !important;" title="Create Bookmark">Create</b>',
                        action: function(e, dt, node, config) {
                            callCreateBookmark();
                        }
                    }
        ...
        var makeGetBookmarksDataTable = function(res_data) {
            var data = ""
            // data += '<button id="delete_bookmark" class="btn btn-map"><b>Hi</b></button>'
			// data += '<br/>'
			// data += '<br/>'
            data += '<table id="bookmarks_table" class="display" style="width:100%;" border="1">'
            // data += '<table id="bookmarks_table" style="width:70%";border="1">'
            data += '<thead><tr>';
            data += '<th style="min-width:50px; text-align:center;">Select</th>';
            data += '<th style="min-width:150px; text-align:center;">Bookmark Name</th>';
            // data += '<th style="text-align:center;">Select</th>';
            // data += '<th style="text-align:center;">Bookmark Name</th>';
            ...
            if (!$.isEmptyObject(res_data)) {
                ...
                $.each(res_data, function(key, record) {
                    console.log(record)
                    data += '<tr>';
                    data += ' <td class="bookmark_checkbox" style=" min-width:50px; text-align:center;"><input type="radio"  style=" background-color:black;color:black" name="bm_option_box" id="' + record.bookmark_name + '" value=' + record.latitude + "," + record.longitude + "," + record.zoom + ' /></td>';
        ...
        var getBookmarks = function() {
            ...
            $.ajax({
                ...
                error: function(returnval) {
                    spinOff()
                    raiseValidationError("Please start node server")
                },
